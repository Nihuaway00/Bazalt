...
export class ChatService {
    ...
    get = async (req, res, next) => {
        try {
            ...
            if (!chatSnap.exists())
                throw ErrorHandler.NotFound("Chat is undefined")
            ...
            const toEncrypt = {
                messages: messageSnaps.map(async snap => {
                    const attachmentSnaps =
                        await AttachmentController.getFromMessageID(snap.id)
                    return {
                        message: snap.data(),
                        attachments: attachmentSnaps.map(attach =>
                            attach.data()
                        ),
                    }
                }),
    ...
    create = async (req, res) => {
        try {
            ...
            if (!title || !userID)
                throw ErrorHandler.BadRequest("no title or userID")

            const re = title.match(/[a-zA-Z0-9]*$/)
            if (!re)
                throw ErrorHandler.BadRequest(
                    "Title is have unsupported symbols"
                )
    ...
    remove = async (req, res) => {
        try {
            ...
            if (chatSnap.data().creatorID !== userID)
                throw ErrorHandler.Forbidden(
                    "You`re not a creator of this chat"
                )
    ...
    invite = async (req, res) => {
        try {
            ...
            const memberSnap = await MemberController.getOne(userID, chat_id)
            if (!memberSnap)
                throw ErrorHandler.Forbidden("You aren`t member of this chat")
            ...
            this.io.to(chat_id).emit("SERVER:chat/member", {
                message: systemMessageSnap.data(),
            })
            res.sendStatus(201)
        } catch (e) {
            console.log(e.message)
    ...
    kick = async (req, res) => {
        try {
            ...
            if (!chatSnap.exists())
                throw ErrorHandler.NotFound("Chat with this ID in not found")
            const toKickMemberSnap = await MemberController.getOne(
                to_kick_id,
                chat_id
            )
            if (!toKickMemberSnap.exists())
                throw ErrorHandler.NotFound("This member is not found")
            const memberSnap = await MemberController.getOne(userID, chat_id)
            if (!memberSnap?.exists())
                throw ErrorHandler.NotFound("You are not member of this chat")
            if (toKickMemberSnap.data().invitedBy !== userID)
                throw ErrorHandler.Forbidden("You weren`t invite this member")
            ...
            this.io.to(chat_id).emit("SERVER:chat/member/kick", {
                message: systemMessageSnap.data(),
            })
            res.sendStatus(201)
        } catch (e) {
    ...
    leave = async (req, res) => {
        try {
            ...
            if (memberSnap.data().userID === chatSnap.data().creatorID) {
                const candidateSnaps = await MemberController.get([
                    where("chatID", "==", chat_id),
                    where("userID", "!=", userID),
                    orderBy("invitedAt", "desc"),
                ])
                if (candidateSnaps.length > 0) {
                    await ChatController.update(chat_id, {
                        creatorID: candidateSnaps[0].id,
                    })
                }
            }
            ...
            if (this.io) {
                this.io.to(chat_id).emit("SERVER:chat/member/leave", {
                    message: systemMessageSnap.data(),
                })
            }
            res.sendStatus(201)
    ...
    editTitle = async (req, res) => {
        try {
            ...
            this.io.to(chat_id).emit("SERVER:chat/edit/title", {
                message: systemMessageSnap.data(),
            })
            res.sendStatus(201)
        } catch (e) {
    ...
    editAvatar = async (req, res) => {
        try {
            ...
            const chatSnap = await ChatController.getFromID(chat_id)
            if (!chatSnap.exists())
                throw ErrorHandler.Forbidden("Chat is undefined")
            const memberSnap = await MemberController.getOne(userID, chat_id)
            if (!memberSnap.exists())
                throw ErrorHandler.Forbidden("You`re not member of this chat")
            ...
            const path = `chat.avatar/${newAvatarRef.id}.${
