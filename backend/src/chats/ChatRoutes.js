...
export const ChatRoutes = (app, io) => {
    ...
    // получение чата
    app.get("/chat/:chat_id", isAuthorized, Chat.get, encryptData)
    ...
    // приглашение в чат
    app.post("/chat/:chat_id/member", isAuthorized, Chat.invite)

    // исключение из чата
    app.post("/chat/:chat_id/member/kick", isAuthorized, Chat.kick)

    // выход из чата
    app.get("/chat/:chat_id/leave", isAuthorized, Chat.leave)
