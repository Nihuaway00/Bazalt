...
export class MessageService {
	...
	remove = async (req, res) => {
		try {
			...
			const messageSnap = await MessageController.getFromID(message_id)
			if (!messageSnap.exists()) throw ErrorHandler.NotFound()
			if (messageSnap.data().system) throw ErrorHandler.Forbidden("Message is system. You cannot delete it")
			if (userID !== messageSnap.data().userID) throw ErrorHandler.Forbidden("You`re not message owner")
	...
	edit = async (req, res) => {
		try {
			...
			const messageSnap = await MessageController.getFromID(message_id)
			if (!messageSnap.exists()) throw ErrorHandler.NotFound()
			if (messageSnap.data().system) throw ErrorHandler.Forbidden("This message is system")
			if (messageSnap.data().userID !== userID) throw ErrorHandler.Forbidden("This is not your message")
			const updatedMessageSnap = await MessageController.update(message_id, { value, editedAt: new Date().valueOf() })

			const chatSnap = await chatController.getFromID(messageSnap.data().chatID)
			const chatKeyJwk = chatSnap.data().key
