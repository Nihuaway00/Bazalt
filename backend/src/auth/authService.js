...
export class AuthService {
    static async login(req, res, next) {
        try {
            ...
            if (
                ...
                throw ErrorHandler.BadRequest("Not enough data for make a safe connection")
            }

            const rsa = new RsaCryptoHandler()
            await rsa.importDecryptKey("jwk", decryptKeyJwk)
            const publicNumSignature = await rsa.decrypt(
                new Uint8Array(publicNumSignatureEncrypted.split(","))
            )

            const hmac = new HmacCryptoHandler()
            await hmac.import("jwk", HMACKeyJwk)
            const verified = await hmac.verify(publicNumSignature, pubNum)
            ...
            res.send({
                ...
                serverPubNum: server_pubNum.toString(),
            })
        } catch (e) {
            next(e)
        }
    ...
    static activate = async (req, res, next) => {
        try {
            ...
            if (!activate_token)
                throw ErrorHandler.BadRequest("token is undefined")
    ...
    static async logout(req, res, next) {
        try {
            ...
            session.destroy(() => {})
            const socketIDs = await SocketController.getFromSessionID(
                session.id
            )
    ...
    static async sendRestoreToken(req, res, next) {
        try {
            ...
            if (!userSnap.exists())
                throw ErrorHandler.BadRequest(
                    "User with this email isn`t exist"
                )
    ...
    static async restoreAccess(req, res, next) {
        try {
            ...
            if (passSnap.data().value === hashPass)
                throw ErrorHandler.BadRequest(
                    "New password is equals to old password"
                )
            ...
            sessionIDs.map(async sessionID => {
                const socketIDs =
                    await SocketController.getFromSessionID(sessionID)
